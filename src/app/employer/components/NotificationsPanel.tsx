    ...
    <>
      ...
      <div className="absolute right-0 top-full mt-2 z-50 w-80 max-w-[90vw] bg-white rounded-2xl shadow-lg border border-[#F0ECE3] overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#F0ECE3]">
          <p className="text-sm font-semibold text-[#1B3A2F]">{t("app_employer_components_notifications_panel.notifications")}</p>
          {hasUnread && (
            <button
              onClick={onMarkAllRead}
              className="text-xs font-medium text-[#C6543A] hover:underline"
            >
              {t("app_employer_components_notifications_panel.mark_all_read")}</button>
          )}
        </div>
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          : notifications.length === 0 ?
            <div className="flex flex-col items-center text-center py-10 px-4">
              ...
              <p className="text-sm text-[#6B7A73]">{t("app_employer_components_notifications_panel.no_notifications_yet")}</p>
            </div>
