  ...
  return (
    ...
    >
      <div className="relative">
        <div
          className={`relative px-3.5 py-2 rounded-2xl text-sm leading-snug ${isDeleted ?
        ...
        >
          {!isDeleted && replyToMessage && (
            <div
              className={`mb-1 pl-2 border-l-2 text-xs opacity-80 truncate max-w-60 ${isMe ? "border-white/50" : "border-black/20"
                }`}
            >
          ...
          <span
            className={`inline-flex items-center gap-1 float-right ml-2 mt-1 translate-y-1 ${isMe ? "text-white/70" : "text-[#9AA79F]"
              }`}
          >
        ...
        {!isDeleted && (
          <div
            className={`absolute top-1/2 -translate-y-1/2 flex items-center gap-0.5
              ...
              transition-opacity ${isMe ? "right-full mr-1.5" : "left-full ml-1.5"
              }`}
          >
            {isMe && (
              <button
                ...
                className="p-1.5 rounded-full text-[#9AA79F] hover:text-[#C6543A] hover:bg-black/5 bg-white shadow-sm border border-black/5"
              >
                <Trash2 size={13} />
              </button>
            )}
            <button
              ...
              aria-label="Reply"
              className="p-1.5 rounded-full text-[#9AA79F] hover:text-[#1B3A2F] hover:bg-black/5 bg-white shadow-sm border border-black/5"
            >
              <Reply size={13} />
            </button>
            <div className="relative">
              ...
              {pickerOpen && (
                <>
                  ...
                  />
                  <div
                    className={`absolute z-50 top-full mt-1 flex items-center gap-1 bg-white rounded-full shadow-lg border border-[#F0ECE3] px-2 py-1.5 ${isMe ? "right-0" : "left-0"
                      }`}
                  >
        ...
        <div className={`flex flex-wrap gap-1 mt-1 ${isMe ? "justify-end" : ""}`}>
          {Object.entries(reactionGroups).map(([emoji, { count, reactedByMe }]) => (
            <button
              ...
              className={`flex items-center gap-1 text-xs rounded-full px-2 py-0.5 border transition-colors ${reactedByMe ?
