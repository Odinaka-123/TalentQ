    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-lg max-h-[85vh] flex flex-col rounded-3xl bg-white shadow-xl">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#EFEBE2]">
          <h2 className="text-base font-semibold text-[#1F2A22]">{t("components_skill_selector_skill_selector_modal.add_skills")}</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
        ...
        <div className="px-6 pt-4">
          ...
          <p className="text-xs text-[#8A8A7E] px-1 py-2">
            {draftSelected.length} {t("components_skill_selector_skill_selector_modal.selected")}</p>
        </div>

        <div className="flex-1 overflow-y-auto px-6 pb-4">
          {filteredCategories.length === 0 && (
            <p className="text-sm text-[#8A8A7E] text-center py-10">
              {t("components_skill_selector_skill_selector_modal.no_skills_found_for")}{query}&quot;
            </p>
          )}
        ...
        <div className="px-6 py-4 border-t border-[#EFEBE2]">
          ...
          >
            {t("components_skill_selector_skill_selector_modal.done")}</button>
        </div>
      </div>
    </div>
  );
